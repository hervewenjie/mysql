...
/********************************************************************//**
对B树进行游标定位时, 主要通过此函数进行, 该函数从根页开始向下层页迭代, 直到指定的层级level,
最终将游标定位在第一个大/小(等于)tuple的位置, 先不考虑页面的latch, 锁, 自适应哈希, 插入缓冲的影响
仅看游标定位
                                                                       
Searches an index tree and positions a tree cursor on a given level.
NOTE: n_fields_cmp in tuple must be set so that it cannot be compared
to node pointer page number fields on the upper levels of the tree!
如果mode是 PAGE_CUR_LE用在insert中
...
/*========================*/
	...
	ulint		mode,           /*!< in: PAGE_CUR_L, ...;
                                    ...
                                    PAGE_CUR_LE to search the position! */ // Insert要用PAGE_CUR_LE模式
	...
	btr_cur_t*	cursor,         /* 光标 */ /*!< in/out: tree cursor; the cursor page is
...
# endif /* BTR_CUR_HASH_ADAPT */
	...
	page_no = dict_index_get_page(index); // 根页的page number
