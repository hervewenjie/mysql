...
// ===================================================================
// 对当前的page进行二分搜索
// 值得注意的是, page_cur_search_with_match对非叶子索引项进行二分搜索时,
// 并不对得到的结果进行检查, 而是直接将结果记录的最后一个自动作为下层页的页号
// 此函数根据传入的tuple大小和比较模式, 将页面游标定位在第一个大/小于(等于)页面的位置
/****************************************************************//**
	...
	page_cur_t*		cursor)     /*!< out: page cursor */
{
	...
	const rec_t*	up_rec;    // 物理记录只是一个byte数组
	const rec_t*	low_rec;
	const rec_t*	mid_rec;
...
# endif
    ...
    // 2. 首先进行页面目录的二分搜索, low为infinmum
	low = 0;
	up = page_dir_get_n_slots(page) - 1; // 返回slot数量

	/* Perform binary search until the lower and upper limit directory
	slots come to the distance 1 of each other */

    // 3. 开始页面的二分搜索, 这个一个非常直接的二分搜索过程
    // Slot稀疏标记线性记录
	while (up - low > 1) {
		mid = (low + up) / 2;
		slot = page_dir_get_nth_slot(page, mid); // 中间的slot
		mid_rec = page_dir_slot_get_rec(slot);   // 中间的记录
...
up_slot_match:
    ...
    // 4. 此时low_rec与up_rec是两个相邻的页面目录槽指向的记录, 然后在low_rec和up_rec之间进行线性搜索
	slot = page_dir_get_nth_slot(page, low);
	low_rec = page_dir_slot_get_rec(slot);  // low_rec记录
	slot = page_dir_get_nth_slot(page, up);
	up_rec = page_dir_slot_get_rec(slot);   // up_rec记录

	/* Perform linear search until the upper and lower records come to
	distance 1 of each other. */

    // page_rec_get_next_const返回下一个记录的指针
...
#endif
    // 6. 将游标正确地定位到页面记录上, 同时记录比较结果
    // page_cur_postion把游标放到某个记录上
	if (mode <= PAGE_CUR_GE) {
		page_cur_position(up_rec, block, cursor);  // GE模式就把游标放到up_rec上
	} else {
		page_cur_position(low_rec, block, cursor); // 其他模式就把游标放到low_rec上
	}
