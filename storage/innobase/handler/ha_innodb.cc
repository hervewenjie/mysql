...
::index_read. The template is used to save CPU time in large joins.
    ...
    我们预编译SELECT进prebuilt->template, 为了在大的join的时候节省CPU时间
...
same table handle instance, if it is a join.
    ...
    执行SELECT, 如果是join, 可能会重复调用index_read
...
table handler in that case has to execute the COMMIT in ::external_lock.
    ...
    SELECT结束的时候, 判断
        trx->n_mysql_tables_in_use为零的时候, 执行COMMIT如果开启了autocommit
        虽然MySQL解释器应该执行autocommit, 但是并没有
        所以table handler在执行的时候需要在external_lock中执行COMMIT
...
{
    printf("根据index来读取一行记录\n");
	ulint		    mode;
	dict_index_t*	index; // 一个index的结构
	ulint           match_mode	= 0;
	int             error;
	dberr_t         ret;
	...
	if (key_ptr) {
		...
		in the index */
        // 如果key_ptr是NULL的话, 就把光标放在第一个或者最后一个index
